	...
	<div style="text-align: center;">
		<form>
			<table class="table table-hover">
				...
				<%
					List<StudentPingJiaScore> list = (List<StudentPingJiaScore>) request
							.getAttribute("Grid_StuInfo");

					for (StudentPingJiaScore score : list) {
				%>
				<tr>
					<td><%=score.getStudent().getName()%></td>
					<td><%=score.getCourse().getCourseName()%></td>
					<td><%=score.getCourse().getTeacher().getTeacherName()%></td>
					<td><%=score.getStudentPingJiaProject().getName()%></td>
					<td><%=score.getStudentPingJiaType().getName()%></td>

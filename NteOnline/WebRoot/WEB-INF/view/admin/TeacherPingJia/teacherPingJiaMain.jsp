...
$('#myModal').on('shown.bs.modal', function () {
  $('#myInput').focus()
	...
	$.ajax({
		...
		type:'post',
		dataType:'JSON',
		url:"${pageContext.request.contextPath}/adminAjax/coursetype1_score_.action?projectid="+project+"&courseid="+course+"&studentid="+student,
		success:function cbf(data)
		{
            ...
            $("#typeTable").append(
                    ...
                    "<td><input style='display:none;' name='courseid' type='text' value='"+course+"'/>" + item[7] + "</td>" + 
                     "<td><input style='display:none;' name='courseid' type='text' value='"+project+"'/>" + item[10] + "</td>" + 
		...
		type:'post',
		dataType:'JSON',
		url:"${pageContext.request.contextPath}/adminAjax/coursetype1_py.action?projectid="+project+"&courseid="+course+"&studentid="+student,
		success:function cbf(data)
		{
			$.each(data, function(i, item) {
...
<body>
    ...
    <ul class="seachform">
      <s:form action="/admin/teacherPingJiaInfo_teacherPingJiaMain.action">
    <li><label>课程名</label>  
    <div class="vocation">
					...
					<tr>
                					...
                					<s:iterator var="projects2" value="projects_score">
                						<s:iterator var="scores_sum" value="sum_score">
                							...
                							<s:if test="#projects2.id==#scores_sum.coursePingJiaProject.id"><td><button data-toggle="modal" data-target="#myModal" onclick="show_(${projects2.id},${scores_sum.student.id},${scores_sum.course.id})">${score }</button><% a=1; %></td></s:if>
                						</s:iterator>
                					<% if(a==0){ %>
